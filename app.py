...
# Configuration de Flask pour utiliser le dossier "sstatic" comme répertoire statique.
app = Flask(__name__, static_folder='sstatic', static_url_path='/static')
app.config['SECRET_KEY'] = 'secret!'
socketio = SocketIO(app, async_mode='eventlet')

# Variables globales
waiting_list = deque(range(1, 3001))
registered_queue = deque()
...
def generate_qr():
    ...
    """
    url = "http://147.93.40.205:5000/register"  # Adaptez l'URL selon votre serveur
    img = qrcode.make(url)
    buf = io.BytesIO()
...
def register():
    ...
    Conserve le numéro en session afin que l'utilisateur garde le même numéro même après rafraîchissement.
    Utilise "ima.png" comme fond d'écran et affiche un logo (images.png) en haut à gauche.
    """
    # Image fixe choisie pour le fond
...
def home():
    """
    Page d'accueil avec un logo (images.png) en haut à gauche et des liens vers les comptoirs.
    """
...
def counter_page(counter_id):
    """
    Page dédiée à un comptoir avec un logo (images.png) en haut à gauche.
    """
    return f"""
    <!DOCTYPE html>
    <html>
